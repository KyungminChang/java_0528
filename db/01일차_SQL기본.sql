...
# = : 같다, != : 다르다, <> : 다르다, is null : null이면, is not null : null이 아니면
# and : ~하고, or : ~이거나
...
# 수정/ 삭제 하려고 하면 수정/삭제를 하지 못하도록 막음
...
delete from student.student where grade = 1 and class= 1 and num = 4;

# delete와 trumcate의 차이
# delete는 데이터만 지우고 설정은 그대로 유지 - 대표적으로 auto_increment 시작 숫자
# truncate은 데이터 뿐만 아니라 설정도 초기로 돌림- auto_increment 숫자가 1에서 시작
# truncate은 다른 테이블이 참조하고 있는 경우 초기화가 안됨
# 1학년 1반 1번 홍길동 학생을 추가alter
...
insert into student.student value(null, 1, 1, 1, '둘리');
...
insert into student.score(midTerm, finalTerm, performance, studentNum)
	value(100, 90, 90, 1, 1);
delete from score;
insert into student.score(midTerm, finalTerm, performance, studentNum)
	value(100, 90, 90, 1, 1);
