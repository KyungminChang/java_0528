...
SELECT CHAR_LENGTH("안녕"); # 6바이트
SELECT CHAR_LENGTH("12"); # 2바이트
...
SELECT FORMAT(1234567.789012, 2);

#BIN(숫자) : 2진수, OCT 8진수 HEX 16진수
...
# SUBSTING(문자열, 시작위치, 길이) : 문자열에서 시작위치부터 길이만큼의 문자열을 반환
...
# HOUR(시간), MINUTE(시간), SECOND(시간)M MICROSECOND(시간) : 시간에서 시/분/초/밀리초 계산
...
# DATEFIFF(시간1, 시간2) : 시간 1에서 시간 2의 차이를 일로 반환. 시간1 - 시간2
# TIMEFIFF(시간1, 시간2) : 시간 1에서 시간 2의 차이를 시:분:초로 반환. 시간1 - 시간2
...
# ABS(숫자) : 절대값
SELECT FLOOR(1.23), CEIL(1.23), ROUND(1.23), ABS(-123);