...
# 워크벤치에서 Schemas 아래 공백 우클릭 후 Create Schema 클릭 후 DB명 입력 후 Aplly
...
#워크스페이스에서 DB명 우클릭 후 Drop Schemas... 클릭 후 Drop now 클릭
...
create table if not exists student.score(
    ...
    foreign key(studentNum) references subject(num) on delete cascade
);
...
# 테이블 수정 - 컬럼 추가
# alter table 테이블명 add 컬러명 타입 [zerofill] [unique] ...;
alter table student.score add test int not null;
# 테이블 수정 - 컬럼 수정
# alter able 테이블명 change 기존컬럼명 새컬럼명 타입 [zerofill] [unique]...;
...
alter table student.score drop aa;